...
const simpleScope = reactive(new workspace.Scope({
  include: [],
  exclude: [],
}))
...
<template>
  <div>
    ...
    <div v-if="showSimple">
      <div class="mt-8">
        <label for="domain" class="block text-sm font-medium text-snow-storm">In Scope Domains <span
            class="text-gray-400">(comma separated, leave blank to allow all domains)</span></label>
        <div class="relative mt-1 rounded-md shadow-sm">
          <input v-model="simpleDomains" @blur="saveSimple" type="text" name="domains" id="domains"
            class="block w-full rounded-md bg-polar-night-4 pr-10 focus:outline-none sm:text-sm" aria-invalid="true"
      ...
      <div class="mt-8">
        <div class="relative flex items-start">
          <div class="flex h-5 items-center">
            <input v-model="includeSubdomains" @change="saveSimple" id="includeSubdomains"
              aria-describedby="includeSubdomains-description" name="includeSubdomains" type="checkbox"
          ...
          <div class="ml-3 text-sm">
            ...
            <p id="includeSubdomains-description" class="text-gray-400">Include all subdomains of the above domain(s) in
              scope.</p>
          </div>
        </div>
      </div>
    ...
    <div v-else>
      <p class="text-sm text-gray-400">A request will be included in scope if ANY <strong><i>include</i></strong> rule
        matches. A request will be excluded if ANY <strong><i>exclude</i></strong> rule matches.
        <strong><i>Exclude</i></strong>
        rules override <strong><i>include</i></strong> rules.
      </p>
